<?php
    ...
    if (empty($_SESSION['_session__'])) {
        ...
        die();
        exit();
    }elseif((isset($_POST['_submit_special_ARPATEAM_'])) OR ($_GET['act']==="delete-kabar")){
        ...
        switch ($_GET['act']) {
            case "add-kabar":
                ...
                // Data file
                $link       = $base_url_admin."/kabar/";
                $penyimpananGambar  = "../../../../assets/files/images/kabar";
                $database   = "kabar";
                // Data file

                $slider     = htmlspecialchars($_POST['___in_slider_special_ARPATEAM']);
                ...
                if ($count>0) {
                    try{
                        $stmt = $pdo->prepare("INSERT INTO $database
                                        (slider,judul,gambar,deskripsi,slug,keyword,description,tgl_terbit,tgl_update,id_sitemap)
                                        VALUES(:slider,:judul,:gambar,:deskripsi,:slug,:keyword,:description,:tgl_terbit,NOW(),:id_sitemap)" );
                                
                        $stmt->bindParam(":slider", $slider, PDO::PARAM_STR);
                        ...
                        $stmt->bindParam(":tgl_terbit", $tgl_terbit, PDO::PARAM_STR);
            ...
            case "edit-kabar":
                ...
                // Data file
                $link               = $base_url_admin."/kabar/".$slug;
                $penyimpananGambar  = "../../../../assets/files/images/kabar";
                // Data file

                $id_sitemap = htmlspecialchars($_POST['___in_id_sitemap_special_ARPATEAM']);
                $id_kabar   = htmlspecialchars($_POST['___in_id_kabar_special_ARPATEAM']);
                $slider     = htmlspecialchars($_POST['___in_slider_special_ARPATEAM']);
                $judul      = htmlspecialchars($_POST['___in_judul_special_ARPATEAM']);
                ...
                try {
                    $sql = "UPDATE $database
                            SET slider          = :slider,
                                ...
                                tgl_update      = NOW()
                            WHERE id_$database  = :id_kabar
                        ";
                                  
                    $statement = $pdo->prepare($sql);

                    $statement->bindParam(":id_kabar", $id_kabar, PDO::PARAM_INT);
                    $statement->bindParam(":slider", $slider, PDO::PARAM_STR);
