...
<head>
    ...
    <!-- Plugins CSS-->
        ...
        <!-- Program, Galeri, Kabar -->
        <?php elseif (($_GET['module']=="program") OR ($_GET['module']=="galeri") OR ($_GET['module']=="kabar")): ?>
            <link href="<?= $base_url_admin ?>/assets/libs/dropzone/min/dropzone.min.css" rel="stylesheet" type="text/css" />
            <link href="<?= $base_url_admin ?>/assets/libs/dropify/css/dropify.min.css" rel="stylesheet" type="text/css" />
        ...
        <!-- Sitemap, Kontak -->
        <?php elseif (($_GET['module']=="sitemap") OR ($_GET['module']=="kontak")): ?>
            <link href="<?= $base_url_admin ?>/assets/libs/datatables.net-bs5/css/dataTables.bootstrap5.min.css" rel="stylesheet" type="text/css" />
            <link href="<?= $base_url_admin ?>/assets/libs/datatables.net-responsive-bs5/css/responsive.bootstrap5.min.css" rel="stylesheet" type="text/css" />
            <link href="<?= $base_url_admin ?>/assets/libs/datatables.net-buttons-bs5/css/buttons.bootstrap5.min.css" rel="stylesheet" type="text/css" />
            <link href="<?= $base_url_admin ?>/assets/libs/datatables.net-select-bs5/css//select.bootstrap5.min.css" rel="stylesheet" type="text/css" />
...
<body class="loading" data-layout='{"mode": "dark", "width": "fluid", "menuPosition": "fixed", "sidebar": { "color": "dark", "size": "default", "showuser": true}, "topbar": {"color": "dark"}, "showRightSidebarOnPageLoad": true}'>
    ...
    <!-- Plugins js-->
        ...
        <?php if (($_GET['module']=="halaman")): ?>
            ...
            <script> 
                new FroalaEditor('#myEditorFroala', {
                    ...
                    imageUploadURL: 'ImgHandlingFroala/',
                    ...
                    events: {
                        'image.removed': function ($img) {
                            ...
                            nw_file_nm      = file_nmx.replace("<?= $base_url; ?>/portal/Admin/appweb/Controllers../../../../../assets/files/others/HandleByFroala/", "");
        ...
        <?php elseif (($_GET['module']=="program") OR ($_GET['module']=="galeri") OR ($_GET['module']=="kabar")): ?>
            ...
            <script>
                function confirmHapusProgram(enkripsi, slug) {
                    Swal.fire({
                        title: 'Apakah anda yakin ingin menghapus Program ini?',
                        text: "Program yang telah terhapus tidak dapat dikembalikan lagi!",
                        showDenyButton: true,
                        ...
                        if (result.isConfirmed) {
                            window.location = "actionDeleteProgram/" + enkripsi;
                        } else if (result.isDenied) {
                            Swal.fire('Program tidak jadi di hapus', '', 'info')
                        }
                    })
                }
                function confirmHapusGaleri(enkripsi, slug) {
                    Swal.fire({
                        title: 'Apakah anda yakin ingin menghapus Galeri ini?',
                        text: "Galeri yang telah terhapus tidak dapat dikembalikan lagi!",
                        showDenyButton: true,
                        confirmButtonText: 'Ya, Hapus Saja',
                        denyButtonText: 'Batal',
                    }).then((result) => {
                        /* Read more about isConfirmed, isDenied below */
                        if (result.isConfirmed) {
                            window.location = "actionDeleteGaleri/" + enkripsi + "/" + slug;
                        } else if (result.isDenied) {
                            Swal.fire('Galeri tidak jadi di hapus', '', 'info')
                        }
                    })
                }
                function confirmHapusKabar(enkripsi, slug) {
                    Swal.fire({
                        title: 'Apakah anda yakin ingin menghapus Kabar ini?',
                        text: "Kabar yang telah terhapus tidak dapat dikembalikan lagi!",
                        showDenyButton: true,
                        confirmButtonText: 'Ya, Hapus Saja',
                        denyButtonText: 'Batal',
                    }).then((result) => {
                        /* Read more about isConfirmed, isDenied below */
                        if (result.isConfirmed) {
                            window.location = "actionDeleteKabar/" + enkripsi;
                        } else if (result.isDenied) {
                            Swal.fire('Kabar tidak jadi di hapus', '', 'info')
                        }
                    })
                }
            </script>
        ...
        <?php elseif (($_GET['module']=="pesan")): ?>
            ...
            <script src="<?= $base_url_admin ?>/assets/js/pages/form-validation.init.js"></script>
            <script>
                function confirmHapusPesan(enkripsi) {
                    Swal.fire({
                        ...
                        /* Read more about isConfirmed, isDenied below */
                        if (result.isConfirmed) {
                            window.location = "actionDeletePesan/" + enkripsi;
                        } else if (result.isDenied) {
        ...
        <?php elseif (($_GET['module']=="sitemap") OR ($_GET['module']=="kontak")): ?>
            ...
            <script>
                ...
                function confirmHapusKotakPesan(enkripsi) {
                    Swal.fire({
                        title: 'Apakah anda yakin ingin menghapus Kotak Pesan ini?',
                        text: "Data yang telah terhapus tidak dapat dikembalikan lagi!",
                        showDenyButton: true,
                        confirmButtonText: 'Ya, Hapus Saja',
                        denyButtonText: 'Batal',
                        ...
                        /* Read more about isConfirmed, isDenied below */
                        if (result.isConfirmed) {
                            window.location = "actionDeleteKotakPesan/" + enkripsi + "/";
                        } else if (result.isDenied) {
                            Swal.fire('Kotak Pesan tidak jadi di hapus', '', 'info')
                        }
                    })
                }
            </script>
        ...
        <!-- All Pages Sweet Alerts js -->
            ...
            <?php elseif ($_SESSION['_alert__']===2): ?>
                <script>
                    Swal.fire({ title: "Hai <?= $_SESSION['_nama__'] ?>, Selamat Datang di Ruang Kerja #LOKALOGI_TEAM", text: "Jangan lupa semangat kerja untuk hari ini!", icon: "info" });
                </script>
